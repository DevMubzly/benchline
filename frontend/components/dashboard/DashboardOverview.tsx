    ...
    <div>
      <div className='flex items-center justify-between mb-8'>
        <div>
          ...
          <p className='text-gray-500 text-sm mt-1'>Your content operating system</p>
        </div>
        <Link href='/projects' className='flex items-center gap-2 bg-black text-white px-4 py-2 text-sm hover:bg-gray-800 transition-colors'>
          <Plus size={16} />
          Add Project
        </Link>
      </div>
      ...
      <div className='grid grid-cols-2 gap-6'>
        <div>
          <h2 className='font-bold text-lg mb-4 border-b border-black pb-2'>Recent GitHub Activity</h2>
          <div className='space-y-3'>
            {recentEvents.length === 0 && (
              <p className='text-gray-400 text-sm'>No activity yet. Connect a project to get started.</p>
            )}
            {recentEvents.map((event) => (
              <div key={event.id} className='border border-gray-200 p-3'>
                <div className='flex items-center gap-2 mb-1'>
                  <span className='text-xs bg-gray-100 px-2 py-0.5 uppercase'>{event.event_type}</span>
                  <span className='text-sm font-medium'>{event.title}</span>
                </div>
                <p className='text-xs text-gray-500'>{event.description?.slice(0, 100)}</p>
        ...
        <div>
          <h2 className='font-bold text-lg mb-4 border-b border-black pb-2'>Recent Content</h2>
          <div className='space-y-3'>
            {contentPieces.length === 0 && (
              <p className='text-gray-400 text-sm'>No content yet. Generate content from your projects.</p>
            )}
            {contentPieces.slice(0, 5).map((piece) => (
              <div key={piece.id} className='border border-gray-200 p-3'>
                <div className='flex items-center gap-2 mb-1'>
                ...
                <p className='text-xs text-gray-500 mt-1'>{piece.body?.slice(0, 80)}...</p>
              </div>
            ))}
          </div>
        </div>
